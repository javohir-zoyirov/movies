...
export const Serials = () => {
  const { moviesData, setDropdown, setSearch, search, dropdown } = useContext(ApiContext);
  ...
  return (
    <div className="container">
      ...
      <div style={{ height: "100%" }} className="row">
        {moviesData?.filter(item => item.Type === "series")
          .filter(item => search === "" || item.Title.toLowerCase().includes(search.toLowerCase()))
          .filter(item => dropdown === "" || item.Country.toLowerCase().includes(dropdown.toLowerCase()) || item.Genre.toLowerCase().includes(dropdown.toLowerCase()))
          .map((item, index) => (
            <div onClick={() => { navigate(`/aboutmovie/${item.imdbID}`); }} key={index} className="col-lg-3 col-md-4 col-sm-6 col-12 mb-4">
